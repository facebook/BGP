...
from neteng.fboss.bgp_attr.thrift_types import TBgpAfi, TIpPrefix
...
def _get_with_fallback_sync(
    canonical_call: Callable[[], bgp_route_types.TCanonicalRibState],
    legacy_call: Callable[[], Sequence[bgp_route_types.TRibEntry]],
) -> list[bgp_route_types.TRibEntry]:
    try:
        return resolve_canonical_rib_state(canonical_call())
    except ApplicationError as error:
        if error.type != ApplicationErrorType.UNKNOWN_METHOD:
            raise
    except ServiceRouterError as error:
...
def get_rib_entries_sync(
    client: TBgpService.Sync,
    afi: TBgpAfi,
) -> list[bgp_route_types.TRibEntry]:
    return _get_with_fallback_sync(
        lambda: client.getRibEntriesCanonical(afi),
        lambda: client.getRibEntries(afi),
    )


def get_rib_prefix_sync(
    client: TBgpService.Sync,
    prefix: str,
) -> list[bgp_route_types.TRibEntry]:
    return _get_with_fallback_sync(
        lambda: client.getRibPrefixCanonical(prefix),
        lambda: client.getRibPrefix(prefix),
    )


async def get_rib_entries(
    client: TBgpService.Async,
    afi: TBgpAfi,
    *,
